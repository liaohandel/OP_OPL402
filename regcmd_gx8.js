	...
	if(kapilist.length > 0 ){
		for(kk in kapilist ){
			...
			if(chkcmd == "REGCMD/KEYSETUP"){
				...
				updatekeysstuatusurl250 = "http://192.168.5.250/API/v2/KeypadUpdate.php"+"?ID="+pdbuffer.setuuid+"&KeypadID="+kapilist[kk].POS+"&Index="+kapilist[kk].GROUP+"&value="+kapilist[kk].STU;
	...
	vcmdapipamcheck(res,cmd,uuid,pos,group,cstu,()=>{
		...
		switch(cmd){//sch subcmd //"f5 20 08 00 02 14 12 34 12 34 13" 14920000A0
			...
			case "SET":
				...
				if(pos == "DEFAUTO"){
					//sys web ui default auto value set up to JSONCMD
					for(jj in pdbuffer.jautocmd.DEFAUTOLIST){
						pdbuffer.jautocmd.DEVLIST[jj] = pdbuffer.jautocmd.DEFAUTOLIST[jj];
					}
