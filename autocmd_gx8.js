...
function setschobj(akey){
	...
	sch_autojob[akey].schobj =  schedule.scheduleJob( rulest , function(){
		...
		//sch_autojob[akey].stid = setTimeout(function(){ f3run(akey,"on")},1000);		
		sch_autojob[akey].stid = setTimeout(function(){ f3run(akey,"schcheck")},10);
    ...
    switch(akey){
        case "GROWLED":
            ...
            if(cmd == "schcheck"){
				...
				sch_autojob.GROWLED.stid = setTimeout(function(){f3run("GROWLED","schcheck")},2*60*1000);//
            }			
            break;
        case "CYCLEFAN":
            if(cmd == "on"){
            ...
            if(cmd == "schcheck"){
				...
				sch_autojob.CYCLEFAN.stid = setTimeout(function(){f3run("CYCLEFAN","schcheck")},2*60*1000);//
            }
            break;
        case "SPRAY":
            if(cmd == "on"){
            ...
            if(cmd == "schcheck"){
				...
				sch_autojob.SPRAY.stid = setTimeout(function(){f3run("SPRAY","schcheck")},2*60*1000);//
            }
            break;
        case "REFRESH":
            if(cmd == "on"){
            ...
            if(cmd == "schcheck"){
				...
				sch_autojob.REFRESH.stid = setTimeout(function(){f3run(REFRESH,"schcheck")},2*60*1000);//
            }
            break;
        case "UV":
            if(cmd == "on"){
            ...
            if(cmd == "schcheck"){
				...
				sch_autojob.UV.stid = setTimeout(function(){f3run(akey,"schcheck")},2*60*1000);//
            }
            break;
        case "PUMP":
            if(cmd == "on"){
            ...
            if(cmd == "schcheck"){
				...
				sch_autojob.PUMP.stid = setTimeout(function(){f3run(akey,"schcheck")},2*60*1000);//
            }
            break;
        case "GROWUPDOWN":
            if(cmd == "on"){
            ...
            if(cmd == "schcheck"){
				...
				sch_autojob.GROWUPDOWN.stid = setTimeout(function(){f3run(akey,"schcheck")},2*60*1000);//
            }
            break;
        case "AIRCON":
            if(cmd == "load"){
...
function active_keypadjob(kpos,kcode,kactive){
	...
	updatekeysstuatusurl250 = "http://192.168.5.250/API/v2/KeypadUpdate.php"+"?ID="+pdbuffer.setuuid+"&KeypadID="+kpos+"&Index="+kcode+"&value="+kactive;
	console.log("sudo active update to webui =>"+updatekeysstuatusurl250);
	client.get(updatekeysstuatusurl250,cargs, function (data, response) {
		console.log("keypad active update to webui   ok ...");
	}).on("error", function(err) {console.log("err for client");}).on('requestTimeout', function (req) {req.abort();});
	//}		
}
...
function alarmchk_load(alarmjob){
	...
	if(alarmjob.CHKCOUNT == 3){
		...
		updateipc_alarmcodeurl= "http://192.168.5.250/API/v2/Alarm.php"+"?ID="+pdbuffer.setuuid+"&POS="+aapos+"&Type="+alarmjob.EPOS[dd].CMD+"&value="+alarmjob.AMCODE+"&Data="+chkval.vmax;
				...
				}else if("OFF" in alarmjob.EPOS[dd].ACTION){
					if(alarmjob.EPOS[dd].ACTION.OFF == alarmjob.CHKMODE){
						//client 
					}
				}else if("AUTO" in alarmjob.EPOS[dd].ACTION){
